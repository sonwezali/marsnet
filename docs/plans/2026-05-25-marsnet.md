...
- [ ] **Step 2: Create `marsnet/node/config.py`**
...
```bash
pytest tests/test_contact_plan.py -v
```

Expected: `ImportError` or multiple `FAILED`.

- [ ] **Step 3: Implement `marsnet/node/contact_plan.py`**
...
```bash
pytest tests/test_bundle_store.py -v
```
...
```bash
pytest tests/test_image_assembler.py -v
```
...
```bash
uvicorn marsnet.dashboard.server:app --host 0.0.0.0 --port 8000
```
...
```bash
python -c "from marsnet.node.crypto import CryptoManager; CryptoManager.generate().save('shared.key')"
```
...
1. Start dashboard: `uvicorn marsnet.dashboard.server:app --host 0.0.0.0 --port 8000`
